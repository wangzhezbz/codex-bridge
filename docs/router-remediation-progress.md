...
### 记录 45：桌面启动与 Router 开关主线程阻塞治理

状态：已完成实现与桌面回归；已生成一个 Windows x64 便携测试包并通过打包后 smoke；纳入 v0.3.2 正式发布，最终发布状态以 GitHub Release 为准。

根因与处理：

- 完整状态读取把 19～23 秒的 Codex 资源探测放在 Electron 主线程，并占用配置事务共享队列；Router 启停因此既等待扫描，又出现整窗无响应。
- 详细状态请求会永久开启全局粘滞标记，使后续所有状态广播反复执行完整扫描。
- 启动时自动项目恢复按现场 9 个根目录逐项等待，最坏额外占用约 72 秒。
- 资源快照现已移入独立 Worker；广播固定使用轻量状态；完整扫描不再占用配置事务协调器；启动时自动项目恢复已取消，手动恢复入口保留。

验证：

- TDD 先稳定复现 3 项旧行为失败；聚焦回归最终 100/100。
- 真实数据 Worker 扫描耗时 20,746ms 时，主线程 25ms 心跳最大间隔 29ms。
- 最终 `npm run check` 退出码 0：语法、完整 Router、Desktop 817/817、历史恢复 14/14 全部通过；desktop smoke 与补丁检查也通过。详细证据见 `docs/router-remediation-record-45.md`。
- 测试包：`dist-artifacts/v0.3.1-performance-test-20260713-123914/CodexBridge-Windows-x64-Portable.zip`；SHA-256 `E328AE27084AD0A422662336EF991E1AA3322B40BBFF9E320567C4CED052AE74`；打包后 smoke 通过。
